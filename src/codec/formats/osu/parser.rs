...
mod tests {
    ...
    fn test_parse_timing_point_sv() {
        let tp = parse_timing_point("21855,-133.333333333333,4,1,1,50,0,0").unwrap();
        assert!(!tp.uninherited);
        assert!((tp.scroll_velocity() - 0.75).abs() < 0.01); // 0.75x SV
    }

    #[test]
    fn test_parse_timing_point_sv_normal() {
        let tp = parse_timing_point("32500,-100,4,1,1,50,0,0").unwrap();
        assert!((tp.scroll_velocity() - 1.0).abs() < 0.01); // 1.0x SV
    }

    #[test]
    fn test_parse_hit_object_tap() {
        let ho = parse_hit_object("402,192,1694,5,0,0:0:0:0:").unwrap();
        assert_eq!(ho.x, 402);
        assert_eq!(ho.time, 1694);
        assert!(ho.is_tap() || ho.is_hold()); // type 5 = circle + new combo
        assert!(ho.end_time.is_none());
    }
    ...
    fn test_column_calculation() {
        ...
        assert_eq!(ho.column(7), 0); // 36 * 7 / 512 = 0

        let ho2 = OsuHitObject {
            x: 475,
            ..ho.clone()
        };
        assert_eq!(ho2.column(7), 6); // 475 * 7 / 512 = 6
    }
}
