...
impl Encoder for McEncoder {
    fn encode(chart: &RoxChart) -> RoxResult<Vec<u8>> {
        serde_json::to_vec_pretty(&Self::to_mc_json(chart))
            .map_err(|e| crate::error::RoxError::InvalidFormat(format!("Malody JSON encode: {e}")))
    }
}
