...
impl McDecoder {
    ...
    pub fn from_mc(mc: &McChart) -> RoxResult<RoxChart> {
        ...
        for i in 0..n_bpm {
            ...
            let mut t = TimingPoint::bpm(time_us, tp.bpm as f32);
            t.signature = tp.sign;
...
mod tests {
    ...
    fn test_decode_minimal_4k() {
        ...
        let chart = <McDecoder as Decoder>::decode(json.as_bytes()).unwrap();
    ...
    fn test_reject_non_key_mode() {
        ...
        assert!(<McDecoder as Decoder>::decode(json.as_bytes()).is_err());
    }
}
